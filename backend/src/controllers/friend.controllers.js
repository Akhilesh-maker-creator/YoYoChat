...
export const getFriends = async( req, res)=>{
    try {
        const user = await User.findOne({ _id: req.user._id}).select("friends").populate("friends", "name bio email profilePic _id")
...
export const acceptFriendRequest = async( req, res)=>{
    try {
       ...
       if(friendRequest.requestReceiver.toString() !== req.user._id){
        return res.status(400).json({ message:"Unauthorized"})
       }
       const sender = User.findById(friendRequest.requestSender)
       if(!sender){
...
export const outgoingFriendReq = async( req, res)=>{
    try {
        ...
        const outgoingFriendRequests = await FriendRequest.find({
            ...
            status:"pending"
        }).populate("requestReceiver", "name email profilePic")

        res.status(201).json(outgoingFriendRequests)
...
export const incomingFriendReq = async( req, res)=>{
    try {
        ...
        res.status(201).json({incomingPendingFriendReqs,incomingAcceptedFriendReqs})
